...
func TestManagerIsReadyNoPools(t *testing.T) {
	t.Parallel()

	fakeClient := newFakeClient()
	manager := NewManager(fakeClient, "kube-system", map[string]string{"app": "exporter"}, "/v1/load")

	// No pools - should not be ready
	if manager.IsReady() {
		t.Error("IsReady should return false when no pools exist")
	}
}

func TestManagerIsReadyWithPool(t *testing.T) {
	t.Parallel()
	...
	err := manager.UpsertPool(ctx, key, map[string]string{"app": "model-server"}, []int32{8080})
	if err != nil {
		t.Fatalf("UpsertPool failed: %v", err)
	}

	// With a pool that has a ready router, IsReady should return true
	// The router is created ready by default
	if !manager.IsReady() {
		t.Error("IsReady should return true when a pool with ready router exists")
	}
}

func TestManagerIsReadyMultiplePools(t *testing.T) {
	t.Parallel()
	...
	// Should be ready since at least one router is ready
	if !manager.IsReady() {
		t.Error("IsReady should return true when multiple pools exist")
	}
}
