...
IMG ?= external-processor:latest
# Container registry
REGISTRY ?= ghcr.io/randomvariable
...
# Build the binary
build:
	go build -ldflags "$(LDFLAGS)" -o bin/external-processor ./cmd/extproc

# Build without version injection (for quick dev builds)
build-dev:
	go build -o bin/external-processor ./cmd/extproc
...
undeploy:
	kubectl delete -f deployments/extproc/extproc.yaml --ignore-not-found=true
...
run:
	go run ./cmd/extproc --kubeconfig=${HOME}/.kube/config
